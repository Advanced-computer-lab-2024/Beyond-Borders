...
const fetchAdminFilteredProducts = async () => {
  ...
  try {
    const response = await axios.get('http://localhost:8000/api/filterSellerProducts', {
...
const handleAccountDeletion = async () => {
  ...
  const fetchAdminNotifications = async () => {
    try {
      ...
      const response = await axios.get('/api/getSellerNotifications', {
  ...
  const checkAdminNotificationsStatus = async () => {
    ...
    try {
      const response = await axios.get('/api/areAllNotificationsReadSeller', {
        params: { username },
      });
  ...
  const markAllAsRead = async () => {
    ...
    try {
      const response = await axios.put('/api/allNotificationsReadSeller', { username });
      if (response.status === 200) {
...
>
  ...
  sx={{
    ...
    minHeight: '85vh', // Adjust height to center and move further up
    paddingTop: '20px', // Move up slightly
  }}
>
  {/* Filter Controls */}
  <Box
    sx={{
      ...
      padding: '15px',
      marginBottom: '15px',
      width: '85%', // Increased width for better alignment
      maxWidth: '1200px', // Adjust width for better centering
      boxShadow: '0px 4px 8px rgba(0, 0, 0, 0.1)',
    }}
  >
    <Typography variant="h6" sx={{ marginBottom: '10px', color: '#192959', fontSize: '18px' }}>
      Filter Products
    </Typography>
    ...
    <Box sx={{ display: 'flex', gap: '10px', flexWrap: 'wrap', justifyContent: 'center' }}>
      <TextField
        ...
        onChange={(e) =>
          setAdminProductFilters({ ...adminProductFilters, name: e.target.value })
        }
      />
      <TextField
        label="Order Date"
        ...
        onChange={(e) =>
          setAdminProductFilters({ ...adminProductFilters, date: e.target.value })
        }
      ...
      <TextField
        ...
        onChange={(e) =>
          setAdminProductFilters({ ...adminProductFilters, month: e.target.value })
        }
  ...
  >
    <Typography variant="h6" sx={{ marginBottom: '10px', color: '#192959', fontSize: '20px' }}>
      Filtered Products
    </Typography>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ backgroundColor: '#192959', color: '#ffffff' }}>
            <th style={{ padding: '10px', border: '1px solid #192959', color: '#ffffff' }}>
              Product Name
            </th>
            <th style={{ padding: '10px', border: '1px solid #192959', color: '#ffffff' }}>
              Quantity
...
export default YSellerSalesPage;
