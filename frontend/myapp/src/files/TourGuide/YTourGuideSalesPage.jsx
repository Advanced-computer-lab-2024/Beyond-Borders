...
function YTourGuideSalesPage() {
  ...
  const fetchTotalTourists = async () => {
    ...
    try {
      const response = await axios.get("/api/getTotalTouristsForTourGuide", {
        params: { username },
      });
  ...
  const fetchItinerariesWithDetails = async () => {
    ...
    try {
      const response = await axios.get(`/api/getallItinerarys`, { params: { AuthorUsername: username } });
  ...
  useEffect(() => {
    ...
    const fetchHighestRevenueItinerary = async () => {
        ...
        try {
          const response = await axios.get('/api/getHighestRevenueItinerary', {
            params: { username },
          });
  
          if (response.data) {
            setHighestRevenueItinerary(response.data.highestRevenueItinerary);
      ...
      const fetchRevenueData = async () => {
        ...
        try {
          // Fetch total revenue
          const totalRevenueResponse = await axios.get('/api/calculateTourGuideRevenue', {
            params: { username },
          });
          setTotalRevenue(totalRevenueResponse.data.totalRevenue);
    
          // Fetch current month revenue
          const currentMonthRevenueResponse = await axios.get('/api/calculateCurrentMonthRevenue', {
            params: { username },
          });
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const response = await axios.get(`/api/getAdvertiserNotificationstg`, {
        params: { username },
      });
  ...
  const checkNotificationsStatus = async () => {
    ...
    try {
      const response = await axios.get('/api/areAllNotificationsReadtg', {
        params: { username },
      });
  ...
  const markAllAsRead = async () => {
    ...
    try {
      const response = await axios.put('/api/allNotificationsReadtg', { username });
      ...
      {/* Main Content Area with 2 Rows */}
        ...
        <Box sx={styles.row}>
            ...
            <Box sx={styles.revenueContainer}>
                ...
                >
                    Your top revenue-generating itinerary in {new Date().toLocaleString('default', { month: 'long' })} is...
                </Typography>
                <EmojiEventsIcon
                    sx={{
                    fontSize: 60,
                ...
                {highestRevenueItinerary ? (
                    ...
                    >
                    No itineraries found for this month.
                    </Typography>
                )}
                </Box>
        ...
        <Box sx={styles.row2}>
            ...
            <Box sx={styles.revenueContainer}>
                ...
                >
                The total number of tourists that have booked your itineraries is...
                </Typography>
                <Typography
                    variant="h3"
                    sx={{
...
export default YTourGuideSalesPage;
